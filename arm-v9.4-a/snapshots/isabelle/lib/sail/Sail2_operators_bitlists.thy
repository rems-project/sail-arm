...
definition uint0  :: \<open>(bitU)list \<Rightarrow> int \<close>  where 
     \<open> uint0 v = ( maybe_failwith (uint_maybe v))\<close> 
  for  "v"  :: "(bitU)list "
...
definition sint0  :: \<open>(bitU)list \<Rightarrow> int \<close>  where 
     \<open> sint0 v = ( maybe_failwith (sint_maybe v))\<close> 
  for  "v"  :: "(bitU)list "
...
\<comment> \<open>\<open>val string_of_bits_subrange : list bitU -> integer -> integer -> string\<close>\<close>
definition string_of_bits_subrange  :: \<open>(bitU)list \<Rightarrow> int \<Rightarrow> int \<Rightarrow> string \<close>  where 
...
definition set_slice  :: \<open> int \<Rightarrow> int \<Rightarrow>(bitU)list \<Rightarrow> int \<Rightarrow>(bitU)list \<Rightarrow>(bitU)list \<close>  where 
     \<open> set_slice (out_len0::ii) (slice_len::ii) out (n::ii) v = (
