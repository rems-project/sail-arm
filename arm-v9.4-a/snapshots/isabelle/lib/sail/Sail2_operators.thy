...
definition arith_op_no0  :: \<open>(int \<Rightarrow> int \<Rightarrow> int)\<Rightarrow> int \<Rightarrow> int \<Rightarrow>(int)option \<close>  where 
...
definition quots_mword_int  :: \<open>('a::len)Word.word \<Rightarrow> int \<Rightarrow>('a::len)Word.word \<close>  where 
     \<open> quots_mword_int l r = ( l sdiv (Word.word_of_int r))\<close> 
  for  "l"  :: "('a::len)Word.word " 
  and  "r"  :: " int "
...
definition duplicate_bit_bv  :: \<open> 'a BitU_class \<Rightarrow> 'a \<Rightarrow> int \<Rightarrow>(bitU)list \<close>  where 
     \<open> duplicate_bit_bv dict_Sail2_values_BitU_a bit1 len = ( replicate_bits_bv 
  (instance_Sail2_values_Bitvector_list_dict dict_Sail2_values_BitU_a) [bit1] len )\<close> 
