...
type_synonym( 'regs, 'a, 'e) monadS =" 'regs sequential_state \<Rightarrow> ( ('a, 'e)state_result * 'regs sequential_state) set "

\<comment> \<open>\<open>val returnS : forall 'regs 'a 'e. 'a -> monadS 'regs 'a 'e\<close>\<close>
definition returnS  :: \<open> 'a \<Rightarrow> 'regs sequential_state \<Rightarrow>(('a,'e)state_result*'regs sequential_state)set \<close>  where 
...
definition bindS  :: \<open>('regs sequential_state \<Rightarrow>(('a,'e)state_result*'regs sequential_state)set)\<Rightarrow>('a \<Rightarrow> 'regs sequential_state \<Rightarrow>(('b,'e)state_result*'regs sequential_state)set)\<Rightarrow> 'regs sequential_state \<Rightarrow>(('b,'e)state_result*'regs sequential_state)set \<close>  where 
     \<open> bindS m f (s :: 'regs sequential_state) = (
  \<Union> (Set.image ((\<lambda>x .  
  (case  x of   (Value a, s') => f a s' | (Ex e, s') => {(Ex e, s')} ))) (m s)))\<close> 
  for  "m"  :: " 'regs sequential_state \<Rightarrow>(('a,'e)state_result*'regs sequential_state)set " 
  and  "f"  :: " 'a \<Rightarrow> 'regs sequential_state \<Rightarrow>(('b,'e)state_result*'regs sequential_state)set " 
  and  "s"  :: " 'regs sequential_state "


\<comment> \<open>\<open>val seqS: forall 'regs 'b 'e. monadS 'regs unit 'e -> monadS 'regs 'b 'e -> monadS 'regs 'b 'e\<close>\<close>
definition seqS  :: \<open>('regs sequential_state \<Rightarrow>(((unit),'e)state_result*'regs sequential_state)set)\<Rightarrow>('regs sequential_state \<Rightarrow>(('b,'e)state_result*'regs sequential_state)set)\<Rightarrow> 'regs sequential_state \<Rightarrow>(('b,'e)state_result*'regs sequential_state)set \<close>  where 
     \<open> seqS m n = ( bindS m ( (\<lambda>x .  
  (case  x of (_ :: unit) => n ))))\<close> 
  for  "m"  :: " 'regs sequential_state \<Rightarrow>(((unit),'e)state_result*'regs sequential_state)set " 
  and  "n"  :: " 'regs sequential_state \<Rightarrow>(('b,'e)state_result*'regs sequential_state)set "


\<comment> \<open>\<open>val readS : forall 'regs 'a 'e. (sequential_state 'regs -> 'a) -> monadS 'regs 'a 'e\<close>\<close>
definition readS  :: \<open>('regs sequential_state \<Rightarrow> 'a)\<Rightarrow> 'regs sequential_state \<Rightarrow>(('a,'e)state_result*'regs sequential_state)set \<close>  where 
...
definition updateS  :: \<open>('regs sequential_state \<Rightarrow> 'regs sequential_state)\<Rightarrow> 'regs sequential_state \<Rightarrow>(((unit),'e)state_result*'regs sequential_state)set \<close>  where 
...
definition failS  :: \<open> string \<Rightarrow> 'regs sequential_state \<Rightarrow>(('a,'e)state_result*'regs sequential_state)set \<close>  where 
...
definition exitS  :: \<open> unit \<Rightarrow> 'regs sequential_state \<Rightarrow>(('a,'e)state_result*'regs sequential_state)set \<close>  where 
     \<open> exitS _ = ( failS (''exit''))\<close>


\<comment> \<open>\<open>val throwS : forall 'regs 'a 'e. 'e -> monadS 'regs 'a 'e\<close>\<close>
definition throwS  :: \<open> 'e \<Rightarrow> 'regs sequential_state \<Rightarrow>(('a,'e)state_result*'regs sequential_state)set \<close>  where 
...
definition try_catchS  :: \<open>('regs sequential_state \<Rightarrow>(('a,'e1)state_result*'regs sequential_state)set)\<Rightarrow>('e1 \<Rightarrow> 'regs sequential_state \<Rightarrow>(('a,'e2)state_result*'regs sequential_state)set)\<Rightarrow> 'regs sequential_state \<Rightarrow>(('a,'e2)state_result*'regs sequential_state)set \<close>  where 
  ...
  for  "m"  :: " 'regs sequential_state \<Rightarrow>(('a,'e1)state_result*'regs sequential_state)set " 
  and  "h"  :: " 'e1 \<Rightarrow> 'regs sequential_state \<Rightarrow>(('a,'e2)state_result*'regs sequential_state)set " 
  and  "s"  :: " 'regs sequential_state "


\<comment> \<open>\<open>val assert_expS : forall 'regs 'e. bool -> string -> monadS 'regs unit 'e\<close>\<close>
definition assert_expS  :: \<open> bool \<Rightarrow> string \<Rightarrow> 'regs sequential_state \<Rightarrow>(((unit),'e)state_result*'regs sequential_state)set \<close>  where 
...
definition early_returnS  :: \<open> 'r \<Rightarrow> 'regs sequential_state \<Rightarrow>(('a,(('r,'e)sum))state_result*'regs sequential_state)set \<close>  where 
...
definition catch_early_returnS  :: \<open>('regs sequential_state \<Rightarrow>(('a,(('a,'e)sum))state_result*'regs sequential_state)set)\<Rightarrow> 'regs sequential_state \<Rightarrow>(('a,'e)state_result*'regs sequential_state)set \<close>  where 
     \<open> catch_early_returnS m = (
  try_catchS m
    ((\<lambda>x .  (case  x of   Inl a => returnS a | Inr e => throwS e ))))\<close> 
  for  "m"  :: " 'regs sequential_state \<Rightarrow>(('a,(('a,'e)sum))state_result*'regs sequential_state)set "
...
definition liftRS  :: \<open>('regs sequential_state \<Rightarrow>(('a,'e)state_result*'regs sequential_state)set)\<Rightarrow> 'regs sequential_state \<Rightarrow>(('a,(('r,'e)sum))state_result*'regs sequential_state)set \<close>  where 
...
definition try_catchRS  :: \<open>('regs sequential_state \<Rightarrow>(('a,(('r,'e1)sum))state_result*'regs sequential_state)set)\<Rightarrow>('e1 \<Rightarrow> 'regs sequential_state \<Rightarrow>(('a,(('r,'e2)sum))state_result*'regs sequential_state)set)\<Rightarrow> 'regs sequential_state \<Rightarrow>(('a,(('r,'e2)sum))state_result*'regs sequential_state)set \<close>  where 
     \<open> try_catchRS m h = (
  try_catchS m
    ((\<lambda>x .  (case  x of   Inl r => throwS (Inl r) | Inr e => h e ))))\<close> 
  for  "m"  :: " 'regs sequential_state \<Rightarrow>(('a,(('r,'e1)sum))state_result*'regs sequential_state)set " 
  and  "h"  :: " 'e1 \<Rightarrow> 'regs sequential_state \<Rightarrow>(('a,(('r,'e2)sum))state_result*'regs sequential_state)set "


\<comment> \<open>\<open>val maybe_failS : forall 'regs 'a 'e. string -> maybe 'a -> monadS 'regs 'a 'e\<close>\<close>
definition maybe_failS  :: \<open> string \<Rightarrow> 'a option \<Rightarrow> 'regs sequential_state \<Rightarrow>(('a,'e)state_result*'regs sequential_state)set \<close>  where 
...
definition chooseS  :: \<open> 'a set \<Rightarrow> 'regs sequential_state \<Rightarrow>(('a,'e)state_result*'regs sequential_state)set \<close>  where 
...
definition choose_boolS  :: \<open> unit \<Rightarrow> 'regs sequential_state \<Rightarrow>(((bool),'e)state_result*'regs sequential_state)set \<close>  where 
     \<open> choose_boolS _ = ( chooseS {False, True})\<close>


\<comment> \<open>\<open>val headS : forall 'rv 'a 'e. list 'a -> monadS 'rv 'a 'e\<close>\<close>
definition headS  :: \<open> 'a list \<Rightarrow> 'rv sequential_state \<Rightarrow>(('a,'e)state_result*'rv sequential_state)set \<close>  where 
     \<open> headS = ( (\<lambda>x .  
  (case  x of   x # _ => returnS x | [] => failS (''headM'') )))\<close>


\<comment> \<open>\<open>val tailS : forall 'rv 'a 'e. list 'a -> monadS 'rv (list 'a) 'e\<close>\<close>
definition tailS  :: \<open> 'a list \<Rightarrow> 'rv sequential_state \<Rightarrow>((('a list),'e)state_result*'rv sequential_state)set \<close>  where 
     \<open> tailS = ( (\<lambda>x .  
...
definition write_mem_bytesS  :: \<open> write_kind \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow>(memory_byte)list \<Rightarrow> 'regs sequential_state \<Rightarrow>(((bool),'e)state_result*'regs sequential_state)set \<close>  where 
...
definition write_memtS  :: \<open> 'a Bitvector_class \<Rightarrow> 'b Bitvector_class \<Rightarrow> write_kind \<Rightarrow> 'a \<Rightarrow> int \<Rightarrow> 'b \<Rightarrow> bitU \<Rightarrow> 'regs sequential_state \<Rightarrow>(((bool),'e)state_result*'regs sequential_state)set \<close>  where 
...
definition write_memS  :: \<open> 'a Bitvector_class \<Rightarrow> 'b Bitvector_class \<Rightarrow> write_kind \<Rightarrow> 'addrsize \<Rightarrow> 'a \<Rightarrow> int \<Rightarrow> 'b \<Rightarrow> 'regs sequential_state \<Rightarrow>(((bool),'e)state_result*'regs sequential_state)set \<close>  where 
...
definition read_regS  :: \<open>('regs,'rv,'a)register_ref \<Rightarrow> 'regs sequential_state \<Rightarrow>(('a,'e)state_result*'regs sequential_state)set \<close>  where 
...
definition write_regS  :: \<open>('regs,'rv,'a)register_ref \<Rightarrow> 'a \<Rightarrow> 'regs sequential_state \<Rightarrow>(((unit),'e)state_result*'regs sequential_state)set \<close>  where 
...
\<comment> \<open>\<open>val show_result : forall 'a 'e. state_result 'a 'e -> string\<close>\<close>
definition show_result  :: \<open>('a,'e)state_result \<Rightarrow> string \<close>  where 
...
\<comment> \<open>\<open>val prerr_results : forall 'a 'e 's. SetType 's => set (state_result 'a 'e * 's) -> unit\<close>\<close>
definition prerr_results  :: \<open>(('a,'e)state_result*'s)set \<Rightarrow> unit \<close>  where 
     \<open> prerr_results rs = (
  (let _ = (Set.image ( (\<lambda>x .  
  (case  x of (r, _) => (let _ = (prerr_endline (show_result r)) in () ) ))) rs) in
  () ))\<close> 
  for  "rs"  :: "(('a,'e)state_result*'s)set "
