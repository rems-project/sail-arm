...
definition duplicate_to_list  :: \<open> 'a \<Rightarrow> int \<Rightarrow> 'a list \<close>  where 
     \<open> duplicate_to_list bit1 length1 = ( repeat [bit1] length1 )\<close> 
  for  "bit1"  :: " 'a " 
  and  "length1"  :: " int "
...
\<comment> \<open>\<open>val hex_str_upper : integer -> string\<close>\<close>

definition hex_str_upper  :: \<open> int \<Rightarrow> string \<close>  where 
     \<open> hex_str_upper i = ( hex_str i )\<close> 
  for  "i"  :: " int "
...
definition access_mword_dec  :: \<open>('a::len)Word.word \<Rightarrow> int \<Rightarrow> bitU \<close>  where 
     \<open> access_mword_dec w n = ( bitU_of_bool (bit w (nat_of_int n)))\<close> 
  for  "w"  :: "('a::len)Word.word " 
  and  "n"  :: " int "
...
definition update_mword_bool_dec  :: \<open>('a::len)Word.word \<Rightarrow> int \<Rightarrow> bool \<Rightarrow>('a::len)Word.word \<close>  where 
     \<open> update_mword_bool_dec w n b = ( set_bit w (nat_of_int n) b )\<close> 
  for  "w"  :: "('a::len)Word.word " 
  and  "n"  :: " int " 
  and  "b"  :: " bool "
...
definition instance_Sail2_values_Bitvector_Machine_word_mword_dict  :: \<open>(('a::len)Word.word)Bitvector_class \<close>  where 
  ...
  bits_of_method = ((\<lambda> v. List.map bitU_of_bool (to_bl v))),

  of_bits_method = ((\<lambda> v. map_option of_bl (just_list (List.map bool_of_bitU v)))),

  of_bools_method = ((\<lambda> v. of_bl v)),
...
\<comment> \<open>\<open>val string_of_bv_subrange : forall 'a. Bitvector 'a => 'a -> integer -> integer -> string\<close>\<close>
definition string_of_bv_subrange  :: \<open> 'a Bitvector_class \<Rightarrow> 'a \<Rightarrow> int \<Rightarrow> int \<Rightarrow> string \<close>  where 
     \<open> string_of_bv_subrange dict_Sail2_values_Bitvector_a v i j = ( show_bitlist (subrange_bv_dec 
  (instance_Sail2_values_Bitvector_list_dict
     instance_Sail2_values_BitU_Sail2_values_bitU_dict) ((bits_of_method   dict_Sail2_values_Bitvector_a) v) i j))\<close> 
  for  "dict_Sail2_values_Bitvector_a"  :: " 'a Bitvector_class " 
  and  "v"  :: " 'a " 
  and  "i"  :: " int " 
  and  "j"  :: " int "
...
let external_slice (d:direction) (start:nat) ((i,j):(nat*nat)) =
  match d with
  \<open> This is the case the thread/concurrency model expects, so no change needed \<close>
  | D_increasing -> (i,j)
