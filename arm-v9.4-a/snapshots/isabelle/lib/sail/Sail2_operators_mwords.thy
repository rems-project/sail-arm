...
definition cons_vec_bool  :: \<open> bool \<Rightarrow>('a::len)Word.word \<Rightarrow>('b::len)Word.word \<close>  where 
     \<open> cons_vec_bool b w = ( of_bl (b # to_bl w))\<close> 
  for  "b"  :: " bool " 
...
definition vec_of_bool  :: \<open> int \<Rightarrow> bool \<Rightarrow>('a::len)Word.word \<close>  where 
     \<open> vec_of_bool _ b = ( of_bl [b])\<close> 
  for  "b"  :: " bool "
...
\<comment> \<open>\<open>val string_of_bits_subrange : forall 'a. Size 'a => mword 'a -> integer -> integer -> string\<close>\<close>
definition string_of_bits_subrange  :: \<open>('a::len)Word.word \<Rightarrow> int \<Rightarrow> int \<Rightarrow> string \<close>  where 
     \<open> string_of_bits_subrange = ( 
  string_of_bv_subrange
    instance_Sail2_values_Bitvector_Machine_word_mword_dict )\<close>
...
definition replicate_bits  :: \<open>('a::len)Word.word \<Rightarrow> int \<Rightarrow>('b::len)Word.word \<close>  where 
     \<open> replicate_bits v count1 = ( of_bl (repeat (to_bl v) count1))\<close> 
  for  "v"  :: "('a::len)Word.word " 
  and  "count1"  :: " int "
...
definition duplicate_bool  :: \<open> bool \<Rightarrow> int \<Rightarrow>('a::len)Word.word \<close>  where 
     \<open> duplicate_bool   b n = ( of_bl (repeat [b] n))\<close> 
  for  "b"  :: " bool " 
  and  "n"  :: " int "
...
definition reverse_endianness  :: \<open>('a::len)Word.word \<Rightarrow>('a::len)Word.word \<close>  where 
     \<open> reverse_endianness v = ( of_bl (reverse_endianness_list (to_bl v)))\<close> 
  for  "v"  :: "('a::len)Word.word "
