...
\<comment> \<open>\<open>val undefined_list : forall 'rv 'a 'e. Register_Value 'rv => 'a -> monad 'rv (list 'a) 'e\<close>\<close>
definition undefined_list  :: \<open> 'rv Register_Value_class \<Rightarrow> 'a \<Rightarrow>('rv,('a list),'e)monad \<close>  where 
     \<open> undefined_list dict_Sail2_values_Register_Value_rv a = (
  choose_nat dict_Sail2_values_Register_Value_rv ((''undefined_list length'')) \<bind> ((\<lambda> n . 
  return (List.replicate (nat (abs ( n))) a))))\<close> 
